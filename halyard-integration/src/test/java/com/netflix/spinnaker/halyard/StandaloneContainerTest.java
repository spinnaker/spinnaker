...
class StandaloneContainerTest {
  ...
  private static GenericContainer halyardContainer;
  ...
  static void setupOnce() throws Exception {
    ...
    halyardContainer =
        new GenericContainer(dockerImageName)
            .withExposedPorts(8064)
            .waitingFor(Wait.forHealthcheck())
            .withEnv("SPRING_APPLICATION_JSON", getSpringApplicationJson());

    Slf4jLogConsumer logConsumer = new Slf4jLogConsumer(logger);
    halyardContainer.start();
    halyardContainer.followOutput(logConsumer);
  }
  ...
  static void cleanupOnce() {
    if (halyardContainer != null) {
      halyardContainer.stop();
    }
  }
    ...
    HttpRequest request =
        HttpRequest.newBuilder()
            .uri(
                new URI(
                    "http://"
                        + halyardContainer.getHost()
                        + ":"
                        + halyardContainer.getFirstMappedPort()
                        + "/health"))
            .GET()
            .build();
