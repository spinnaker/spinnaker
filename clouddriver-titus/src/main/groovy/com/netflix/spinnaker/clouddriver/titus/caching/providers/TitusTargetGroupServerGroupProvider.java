...
public class TitusTargetGroupServerGroupProvider implements TargetGroupServerGroupProvider {
  ...
  @Override
  public Map<String, AmazonTargetGroup> getServerGroups(
    ...
    Map<String, Map> instances =
        cacheView.getAll(INSTANCES.ns, instanceKeys).stream()
  ...
  private Optional<LoadBalancerInstance> getInstanceHealth(
    ...
    try {
      healthKey =
          com.netflix.spinnaker.clouddriver.aws.data.Keys.getInstanceHealthKey(
  ...
  private static Map<String, Object> getTargetGroupHealth(
      String instanceKey, Map<String, String> targetGroupDetails, CacheData healthData) {
    try {
  ...
  private static Map getTargetGroupHealthData(
      Map<String, String> targetGroupDetails, CacheData healthData) {
            ...
            .filter(
                tgh ->
                    ((Map) tgh)
                        .get("targetGroupName")
                        .toString()
                        .equals(targetGroupDetails.get("targetGroup")))
            .findFirst()
