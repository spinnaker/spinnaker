...
@Component
public class AliCloudImageTagger extends ImageTagger implements CloudProviderAware {
  private static final Set<String> BUILT_IN_TAGS =
      new HashSet<>(Arrays.asList("appversion", "build_host"));

  @Autowired
  public AliCloudImageTagger(OortService oortService, ObjectMapper objectMapper) {
          ...
          ImmutableMap.<String, Map>builder()
              .put(
                  ...
                  ImmutableMap.builder()
                      ...
                      .put("region", stage.getContext().get("region").toString())
                      .build())
              .build());

      ArrayList<Map> tagList = (ArrayList<Map>) matchedImage.attributes.get("tags");
      Map<String, String> map = new HashMap<>(16);
      for (Map tag : tagList) {
        map.put(tag.get("tagKey").toString(), tag.get("tagValue").toString());
      }
      matchedImage.tags = map;
  ...
  public boolean areImagesTagged(
    ...
    for (Image targetImage : targetImages) {
      ...
      if (matchedImage == null) {
        ...
        ArrayList<Map> tags = (ArrayList<Map>) matchedImage.attributes.get("tags");
        Map<String, String> map = new HashMap<>(16);
        for (Map tag : tags) {
          map.put(tag.get("tagKey").toString(), tag.get("tagValue").toString());
        }
