...
interface SecurityGroupUpserter {
  ...
  static class OperationContext {
    /**
     * The list of operations to send to Clouddriver. Each operation should be a single entry map keyed by the operation
     * name, with the operation map as the value. (A list of maps of maps? We must go deeper...)
     */
    List<Map> operations
