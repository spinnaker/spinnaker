...
class UpdateLaunchConfigTaskSpec extends Specification {
  ...
  @Subject
  UpdateLaunchConfigTask task = new UpdateLaunchConfigTask(operationsRunner, "default")
  ...
  void "should populate deploy.server.groups to enable force cache refresh"() {
    ...
    then:
    1 * operationsRunner.run(_) >> new KatoOperationsContext(new TaskId("blerg"), null)
  ...
  void "should add amiName from upstream task if not specified"() {
    ...
    1 * operationsRunner.run(_) >> {
      ...
      def opConfig = operationsInput.getOperations().last().updateLaunchConfig
      assert opConfig.amiName == expectedAmi
      new KatoOperationsContext(new TaskId("blerg"), null)
    }
  ...
  def "prefers the ami from an upstream stage to one from deployment details"() {
    ...
    interaction {
      ...
      operationsRunner.run(_) >> {
        OperationsInput operationsInput = it[0]
        operations.addAll(operationsInput.getOperations().flatten())
  ...
  void "should inject allowLaunch if deploy account does not match bake account"() {
    ...
    1 * operationsRunner.run(_) >> {
      OperationsInput operationsInput = it[0]
      def ops = operationsInput.getOperations()
