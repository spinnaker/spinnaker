...
class ImageService {
  ...
  List<Map> search(String provider, String query, String region) {
    HystrixFactory.newListCommand(GROUP, "images-${provider}-${query}-${region}", true) {
      oortService.findImages(provider, query, region)
    } execute()
  }
}
