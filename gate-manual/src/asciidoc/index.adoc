...
This payload should be an HTTP `POST` request to the `/applications/myapp/tasks` endpoint of the Spinnaker Gateway API. Since all orchestrations within Spinnaker are asynchronous, this operation will return a task reference, which can be polled for successful completion.
