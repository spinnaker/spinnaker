...
class DeleteAsgDescriptionValidatorSpec extends Specification {

  @Shared
  DeleteAsgDescriptionValidator validator

  void setupSpec() {
    validator = new DeleteAsgDescriptionValidator()
  ...
  void "empty description fails validation"() {
    setup:
    def description = new DeleteAsgDescription()
  ...
  void "region is validates against configuration"() {
    setup:
    def description = new DeleteAsgDescription(regions: ["us-east-5"])
    def errors = Mock(Errors)
