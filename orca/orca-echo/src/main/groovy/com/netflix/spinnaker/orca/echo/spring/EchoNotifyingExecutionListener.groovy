...
class EchoNotifyingExecutionListener implements ExecutionListener {
  ...
  private final RunAsTokenProvider runAsTokenProvider
  ...
  EchoNotifyingExecutionListener(
    ...
    this.runAsTokenProvider = runAsTokenProvider
  ...
  /**
   * Stamp a signed identity token onto the current (run-as) thread context so the Front50
   * notifications lookup authenticates as the execution's user. The execution-complete listener
   * thread otherwise carries only an unsigned X-SPINNAKER-USER header (no signed token), which under
   * owner-local authorization is roleless and denied. Best effort: re-issue a fresh token from the
   * subject + roles captured at admission, and do nothing when no provider or subject is available.
   */
  private void stampIdentityToken(PipelineExecution pipeline) {
    if (runAsTokenProvider == null) {
      return
    }
    def authentication = pipeline.authentication
    String subject = authentication?.user
    if (!subject) {
      return
    }
    runAsTokenProvider
      .issueExecutionToken(
        ...
        authentication.accountManager)
      .ifPresent { String token -> AuthenticatedRequest.set(Header.IDENTITY_TOKEN, token) }
  }
