...
class DetermineSourceServerGroupTaskSpec extends Specification {
  ...
  void 'should include source in context'() {
    given:
    StageExecutionImpl stage = new StageExecutionImpl(PipelineExecutionImpl.newPipeline("orca"), 'deploy', 'deploy', [
      account          : account,
      application      : 'foo',
  ...
  void 'should useSourceCapacity from context if not provided in Source'() {
    given:
    StageExecutionImpl stage = new StageExecutionImpl(PipelineExecutionImpl.newPipeline("orca"), 'deploy', 'deploy', [useSourceCapacity: contextUseSourceCapacity, account: account, application: application, availabilityZones: [(region): []]])
