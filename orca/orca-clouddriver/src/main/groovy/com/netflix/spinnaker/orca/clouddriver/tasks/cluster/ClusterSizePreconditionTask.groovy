...
class ClusterSizePreconditionTask implements CloudProviderAware, RetryableTask, PreconditionTask {
  ...
  Cluster readCluster(ComparisonConfig config, String credentials, String cloudProvider) {
    def response
    try {
      response = Retrofit2SyncCall.execute(oortService.getCluster(config.application, credentials, config.cluster, cloudProvider))
    ...
    return objectMapper.readValue(response.byteStream(), Cluster)
  }
