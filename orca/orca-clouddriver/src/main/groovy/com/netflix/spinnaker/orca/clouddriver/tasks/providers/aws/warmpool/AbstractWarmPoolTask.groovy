...
@Slf4j
abstract class AbstractWarmPoolTask implements CloudProviderAware, Task {
  @Autowired
  ...
  TaskResult execute(@Nonnull StageExecution stage) {
    ...
    if (TargetServerGroup.isDynamicallyBound(stage)) {
      ...
      // because each region desired should have been spun off its own stage for that region.
      targetServerGroup = resolver.resolve(stage)[0]
    }
    def asgName = targetServerGroup.name
    def region = targetServerGroup.region

    def stageContext = new HashMap(stage.context) + [
      asgs: [[serverGroupName: asgName, region: region]]
    ]
