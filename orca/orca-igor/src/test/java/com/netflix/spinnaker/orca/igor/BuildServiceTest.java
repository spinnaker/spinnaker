...
public class BuildServiceTest {
  ...
  private static void registerProperties(DynamicPropertyRegistry registry) {
    registry.add("wiremock.server.port", wmIgor::getPort);
  }
  ...
  @Autowired DefaultServiceClientProvider defaultServiceClientProvider;

  @BeforeEach
  public void init() {
    IgorService igorService =
        defaultServiceClientProvider.getService(
            IgorService.class,
            new DefaultServiceEndpoint("igor", RetrofitUtils.getBaseUrl(wmIgor.baseUrl())),
            new ObjectMapper());
  ...
  public void testBuildServiceBuildApi() {
    wmIgor.stubFor(
        WireMock.put(
                urlEqualTo(
                    ENDPOINT_WITH_INCORRECT_ENCODING)) // FIXME: the query params should be encoded
            // with spaces, and not with +
