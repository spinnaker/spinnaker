...
class GoogleResourceRetriever {
  ...
  private static void load(AccountCredentialsProvider accountCredentialsProvider) {
    ...
    for (GoogleCredentials credentials : getAllGoogleCredentialsObjects(accountCredentialsProvider)) {
      ...
      for (def pool : poolsListResult.resources) {
        ...
        if (appName) {
          if (!tempAppMap[appName]) {
            ...
            tempAppMap[appName].clusterNames[ACCOUNT_NAME] = new HashSet<String>()
            tempAppMap[appName].clusters[ACCOUNT_NAME] = new HashMap<String, GoogleCluster>()
          }

          if (!tempAppMap[appName].clusters[ACCOUNT_NAME][names.cluster]) {
            tempAppMap[appName].clusters[ACCOUNT_NAME][names.cluster] = new GoogleCluster(name: names.cluster, accountName: ACCOUNT_NAME)
          }

          def cluster = tempAppMap[appName].clusters[ACCOUNT_NAME][names.cluster]
  ...
  private static Set<GoogleCredentials> getAllGoogleCredentialsObjects(AccountCredentialsProvider accountCredentialsProvider) {
    def allGoogleCredentialsSet = new HashSet<GoogleCredentials>()
    ...
    for (def accountCredentials : accountCredentialsProvider.getAll()) {
      try {
        if (accountCredentials.credentials instanceof GoogleCredentials) {
          allGoogleCredentialsSet.add(accountCredentials.credentials)
        }
      } catch (e) {
