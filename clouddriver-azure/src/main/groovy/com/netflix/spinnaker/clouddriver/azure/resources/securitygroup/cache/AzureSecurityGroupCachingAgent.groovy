...
@Slf4j
class AzureSecurityGroupCachingAgent implements CachingAgent, AccountAware {
  ...
  CacheResult loadData(ProviderCache providerCache) {
    ...
    def securityGroups = creds.networkClient.getNetworkSecurityGroupsAll(creds, region)

    List<CacheData> data = securityGroups.collect() { AzureSecurityGroupDescription securityGroup ->
      Map<String, Object> attributes = [ securitygroup: securityGroup]
      new DefaultCacheData(Keys.getSecurityGroupKey(azureCloudProvider, securityGroup.name, securityGroup.id, region, accountName),
        attributes,
        [:])
    }
