...
public class EchoServiceHealthIndicator implements HealthIndicator {
    ...
    @Autowired
    EchoServiceHealthIndicator(Registry registry,
                               Optional<EchoService> echoService) {
        this.echoService = echoService;
        this.upOnce = !echoService.isPresent() ? new AtomicBoolean(true) : new AtomicBoolean(false);
        this.errors = PolledMeter.using(registry)
            .withName("health.echo.errors")
    ...
    public Health health() {
        ...
        Optional.ofNullable(lastException.get()).ifPresent(le -> {
