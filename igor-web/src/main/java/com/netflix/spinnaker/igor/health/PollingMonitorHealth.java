...
/**
 * The health status is based on whether the poller is running and how long ago it last polled. If 5 times the polling
...
public class PollingMonitorHealth implements HealthIndicator {
    ...
    public PollingMonitorHealth(Registry registry, ApplicationContext applicationContext) {
        ...
        this.downPollers = PolledMeter.using(registry)
            .withName("health.pollers.down")
    ...
    public Health health() {
        ...
        List<PollingMonitor> downPollingMonitors = pollers
            .stream()
        ...
        if (upOnce.get() && !downPollingMonitors.isEmpty()) {
            final List<Health> healths = pollers
                .stream()
                .map(this::getPollingMonitorHealth).collect(toList());
    ...
    private Health getPollingMonitorHealth(PollingMonitor pollingMonitor) {
        ...
        if (elapsed > 5 * pollingMonitor.getPollInterval() * DateTimeConstants.MILLIS_PER_SECOND) {
            ...
            return Health.down()
                ...
                .withDetail(String.format("%s.lastPoll", pollingMonitor.getName()), pollingMonitor.getLastPoll())
                .build();
        }
