...
public class JenkinsService implements BuildOperations, BuildProperties {
    ...
    public ProjectsList getProjects() {
        ProjectsList projectsList = new SimpleJava8HystrixCommand<>(
            groupKey, buildCommandKey("getProjects"), jenkinsClient::getProjects).execute();

        if (projectsList == null || projectsList.getList() == null) {
            return new ProjectsList();
        }
        List<Project> projects = projectsList.getList().stream()
    ...
    private Stream<Project> recursiveGetProjects(Project project, String prefix) {
        ...
        return project.getList().stream()
            .flatMap(p -> recursiveGetProjects(p, projectName + "/job/"));
    }


    public JobList getJobs() {
        return new SimpleJava8HystrixCommand<>(groupKey, buildCommandKey("getJobs"), jenkinsClient::getJobs).execute();
    }

    public String getCrumb() {
        if (csrf) {
            Crumb crumb = new SimpleJava8HystrixCommand<>(groupKey, buildCommandKey("getCrumb"), jenkinsClient::getCrumb).execute();
            if (crumb != null) {
    ...
    public BuildDependencies getDependencies(String jobName) {
        return new SimpleJava8HystrixCommand<>(
            groupKey, buildCommandKey("getDependencies"), () -> jenkinsClient.getDependencies(encode(jobName))).execute();
    }
    ...
    public int triggerBuildWithParameters(String job, Map<String, String> queryParameters) {
        ...
        String queuedLocation = response.getHeaders().stream()
            .filter(h -> h.getName() != null)
            .filter(h -> h.getName().toLowerCase().equals("location"))
            .map(Header::getValue)
            .findFirst()
            .orElseThrow(() -> new QueuedJobDeterminationError("Could not find Location header for job '" + job + "'"));
    ...
    private ScmDetails getGitDetails(String jobName, Integer buildNumber) {
        return retrySupport.retry(() -> new SimpleJava8HystrixCommand<>(
