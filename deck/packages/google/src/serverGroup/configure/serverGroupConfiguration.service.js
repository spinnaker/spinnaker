...
import { GceTagManager } from './wizard/securityGroups/tagManager.service';
...
export class GceServerGroupConfigurationService {
  constructor(
    ...
    const gceTagManager = new GceTagManager();
    const gceLoadBalancerSetTransformer = new GceLoadBalancerSetTransformer();
    ...
    function refreshHealthChecks(command, skipCommandReconfiguration) {
      ...
      return gceHealthCheckReader.listHealthChecks().then(function (healthChecks) {
    ...
    function refreshInstanceTypes(command) {
      InfrastructureCaches.clearCache('instanceTypes');
    ...
    function attachEventHandlers(cmd) {
      cmd.regionalChanged = function regionalChanged(command) {
        ...
        if (command.regional) {
          ...
          Object.assign(result.dirty, configureZones(command).dirty);
        }
      ...
      cmd.regionChanged = function regionChanged(command) {
        ...
        if (command.region) {
          Object.assign(result.dirty, configureInstanceTypes(command).dirty);
          Object.assign(result.dirty, configureZones(command).dirty);
          Object.assign(result.dirty, configureCpuPlatforms(command).dirty);
          // TODO: Internal Load Balancers also need to be filtered by network.
          Object.assign(result.dirty, configureLoadBalancerOptions(command).dirty);
      ...
      cmd.credentialsChanged = function credentialsChanged(command) {
        ...
        if (command.credentials) {
          ...
          if (!backingData.filtered.regions.includes(command.region)) {
            ...
            Object.assign(result.dirty, command.regionChanged(command).dirty);
          }
          ...
          if (!backingData.filtered.networks.includes(command.network)) {
            ...
            Object.assign(result.dirty, command.networkChanged(command).dirty);
          }

          Object.assign(result.dirty, configureHealthChecks(command).dirty);
          Object.assign(result.dirty, configureInstanceTypes(command).dirty);
      ...
      cmd.networkChanged = function networkChanged(command) {
        ...
        Object.assign(result.dirty, configureSubnets(command).dirty);
        Object.assign(result.dirty, configureSecurityGroupOptions(command).dirty);
      ...
      cmd.zoneChanged = function zoneChanged(command) {
        ...
        command.viewState.dirty = command.viewState.dirty || {};
        Object.assign(command.viewState.dirty, result.dirty);
        Object.assign(command.viewState.dirty, configureInstanceTypes(command).dirty);
        Object.assign(command.viewState.dirty, configureCpuPlatforms(command).dirty);
        Object.assign(command.viewState.dirty, configureAccelerators(command).dirty);
        return result;
      };
      ...
      cmd.customInstanceChanged = function customInstanceChanged(command) {
        ...
        command.viewState.dirty = command.viewState.dirty || {};
        Object.assign(result, command.viewState.dirty, configureCustomInstanceTypes(command).dirty);
