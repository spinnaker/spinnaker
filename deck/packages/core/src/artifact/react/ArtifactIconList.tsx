...
export const ArtifactIconList = (props: IArtifactIconListProps): any => {
  return props.artifacts.map((artifact, i) => {
    ...
    let artifactLabel = artifactName(artifact, name);
    if (isString(type) && type.length > 0) {
      ...
      if (k8sKindMatch != null && k8sKindMatch[1].length > 0) {
        ...
        name = `${kind} ${name}`;
        artifactLabel = artifactName(artifact, name);
      }
    }
    const title = artifactLabel;
    const copyToClipboardText = artifactDelimiter(artifact) === ':' ? artifactLabel : '';
    return (
      <div key={key} className="artifact-list-row">
        <div className="artifact-list-item" title={title}>
          {iconPath && <img className="artifact-list-item-icon" width="20" height="20" src={iconPath} />}
          <span className="artifact-list-item-name">{artifactLabel}</span>
        </div>
