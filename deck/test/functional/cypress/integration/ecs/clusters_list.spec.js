...
describe('Amazon ECS: aws-prod-ecsdemo cluster', () => {
  beforeEach(() => {
    ...
    cy.intercept('/instances/ecs-my-aws-devel-acct/us-west-2/f8757e00-184d-4288-b535-4124a739e7be', {
  ...
  it('shows stored ECS cluster with their sequences', () => {
    ...
    cy.get('.sub-group').first().should('contain.text', 'aws-final-test');
  });
  ...
  it('shows stored instance details view action', () => {
    ...
    cy.get('[data-test-id="instanceDetails.content"]').get('.collapsible-section').eq(1).get('span:contains("Up")');

    cy.get('[data-test-id="instanceDetails.content"]')
      .get('.collapsible-heading')
      .eq(2)
      .click()
