...
import static com.netflix.spinnaker.clouddriver.oraclebmcs.cache.Keys.Namespace.SECURITY_GROUPS

@Slf4j
class OracleBMCSSecurityGroupCachingAgent extends AbstractOracleBMCSCachingAgent implements OnDemandAgent {
  ...
  OracleBMCSSecurityGroupCachingAgent(String clouddriverUserAgentApplicationName, OracleBMCSNamedAccountCredentials credentials, ObjectMapper objectMapper, Registry registry) {
    super(objectMapper, credentials, clouddriverUserAgentApplicationName)
    this.metricsSupport = new OnDemandMetricsSupport(registry, this, "${OracleBMCSCloudProvider.ID}:${OnDemandAgent.OnDemandType.SecurityGroup}")
  }
  ...
  boolean handles(OnDemandAgent.OnDemandType type, String cloudProvider) {
    type == OnDemandAgent.OnDemandType.SecurityGroup && cloudProvider == OracleBMCSCloudProvider.ID
  }
