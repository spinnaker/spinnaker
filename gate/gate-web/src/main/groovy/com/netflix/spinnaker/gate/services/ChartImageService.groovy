...
class ChartImageService {
  ...
  List<Map> getForAccountAndRegion(String provider, String account, String region, String imageId, String selectorKey) {
    Retrofit2SyncCall.execute(clouddriverServiceSelector.select().getChartImageDetails(provider, account, region, imageId))
  }
  ...
  List<Map> search(String provider, String query, String region, String account, Integer count, Map<String, String> additionalFilters, String selectorKey) {
    Retrofit2SyncCall.execute(clouddriverServiceSelector.select().findChartImages(provider, query, region, account, count, additionalFilters))
  }
