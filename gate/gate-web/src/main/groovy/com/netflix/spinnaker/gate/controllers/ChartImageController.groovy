...
class ChartImageController {
  ...
  List<Map> getImageDetails(@PathVariable(value = "account") String account,
    ...
    chartImageService.getForAccountAndRegion(provider, account, region, imageId, sourceApp)
  }

  @Operation(summary = "Retrieve a list of helm chart images, filtered by cloud provider, region, and account",
  ...
  List<Map> findImages(@RequestParam(value = "provider", defaultValue = "aws", required = false) String provider,
    ...
    chartImageService.search(provider, query, region, account, count, additionalFilters, httpServletRequest.getHeader("X-RateLimit-Header"))
  }
  ...
  List<String> findTags(@RequestParam(value = "provider", defaultValue = "aws", required = false) String provider,
                        ...
                        @RequestHeader(value = "X-RateLimit-App", required = false) String sourceApp) {
    chartImageService.findTags(provider, account, repository, sourceApp)
  }
}
