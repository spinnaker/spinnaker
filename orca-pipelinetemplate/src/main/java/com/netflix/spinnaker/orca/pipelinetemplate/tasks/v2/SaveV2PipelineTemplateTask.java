  ...
  default void validate(V2PipelineTemplate template) {
    ...
    if (!template.getMetadata().getName().matches(TEMPLATE_VALID_NAME_REGEX)) {
      throw new IllegalArgumentException(
        String.format("Illegal template name: %s. Name must match the regex: %s",
    ...
    if (template.getVariables() != null) {
      List<String> invalidVariableNames = template.getVariables()
        .stream()
        .filter(variable -> variable.getName() != null && !variable.getName().matches(TEMPLATE_VALID_VARIABLE_NAME_REGEX))
