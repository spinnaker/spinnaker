...
class CatsSearchProvider implements SearchProvider {
  ...
  static Map<String, Template> urlMappings = [
    (SERVER_GROUPS.ns):
      urlMappingTemplateEngine.createTemplate('/applications/${application.toLowerCase()}/clusters/$account/$cluster/titan/serverGroups/$serverGroup?region=$region'),
    (LOAD_BALANCERS.ns):
      urlMappingTemplateEngine.createTemplate('/titan/loadBalancers/$loadBalancer'),
    (CLUSTERS.ns):
  ...
  private Collection<String> buildFilterIdentifiers(AccountCredentialsProvider accountCredentialsProvider, String cache, String query) {
    switch (cache) {
      case INSTANCES.ns:
        ...
        Set<NetflixTitanCredentials> titanCredentials = accountCredentialsProvider.all.findAll {
          it instanceof NetflixTitanCredentials
        } as Set<NetflixTitanCredentials>

        def possibleInstanceIdentifiers = []
        titanCredentials.each { NetflixTitanCredentials credentials ->
          credentials.regions.each { NetflixTitanCredentials.Region region ->
