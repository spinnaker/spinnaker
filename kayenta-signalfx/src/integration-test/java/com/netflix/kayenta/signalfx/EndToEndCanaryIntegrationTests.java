...
public class EndToEndCanaryIntegrationTests extends BaseSignalFxIntegrationTest {
  ...
  @Test
  public void test_that_signalfx_can_be_used_as_a_data_source_for_a_canary_execution_unhealthy()
      throws IOException, InterruptedException {
  ...
  private ValidatableResponse doCanaryExec(String expScope)
      throws IOException, InterruptedException {
    ...
    CanaryClassifierThresholdsConfig canaryClassifierThresholdsConfig =
        CanaryClassifierThresholdsConfig.builder().marginal(50D).pass(75D).build();
    ...
    CanaryScope control =
        new CanaryScope()
            ...
            .setExtendedScopeParams(ImmutableMap.of("test-id", testId))
    ...
    CanaryScope experiment =
        new CanaryScope()
            .setScope(expScope)
            .setExtendedScopeParams(ImmutableMap.of(TEST_ID, testId))
            .setLocation(LOCATION)
            .setStep(1l)
            .setStart(metricsReportingStartTime)
            .setEnd(end);
    ...
    do {
      response =
          when()
              .get(String.format(getUriTemplate(), "/canary/" + canaryExecutionId))
              .then()
              .statusCode(200);
      Thread.sleep(1000);
