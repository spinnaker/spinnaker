...
class StandaloneContainerTest {
  ...
  // redis isn't required for igor to start, but redis starts quickly enough
  ...
  private static GenericContainer igorContainer;
  ...
  static void setupOnce() throws Exception {
    ...
    igorContainer.start();
    igorContainer.followOutput(logConsumer);
  }
    ...
    HttpRequest request =
        HttpRequest.newBuilder()
            .uri(
                new URI(
                    "http://"
                        + igorContainer.getHost()
                        + ":"
                        + igorContainer.getFirstMappedPort()
                        + "/health"))
            .GET()
            .build();
