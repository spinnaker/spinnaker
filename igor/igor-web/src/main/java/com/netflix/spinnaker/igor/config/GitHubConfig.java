...
public class GitHubConfig {
    ...
    @Bean
    public GitHubMaster gitHubMasters(@Valid GitHubProperties gitHubProperties, ObjectMapper mapper, OkHttp3ClientConfiguration okHttpClientConfig) {
        log.info("bootstrapping {} as github", gitHubProperties.getBaseUrl());
        GitHubMaster master = new GitHubMaster();
        master.setGitHubClient(gitHubClient(okHttpClientConfig, gitHubProperties.getBaseUrl(), gitHubProperties.getAccessToken(), mapper));
        master.setBaseUrl(gitHubProperties.getBaseUrl());
        return master;
    }

    public GitHubClient gitHubClient(OkHttp3ClientConfiguration okHttpClientConfig, String address, String accessToken, ObjectMapper mapper) {
