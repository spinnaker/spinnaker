 ...
 */
@Slf4j
public class GitHubMaster extends AbstractScmMaster {
  ...
  public String getTextFileContents(String projectKey, String repositorySlug, String path, String ref) {
    try {
      ...
      if (!FILE_CONTENT_TYPE.equals(response.getType())) {
        throw new NotFoundException("Unexpected content type: " + response.getType());
      }
      return new String(Base64.getMimeDecoder().decode(response.getContent()));
    } catch (SpinnakerNetworkException e) {
      throw new NotFoundException("Could not find the server " + baseUrl);
    } catch (SpinnakerServerException e) {
      log.error(
  ...
  @Override
  public Commit getCommitDetails(String projectKey, String repositorySlug, String sha) {
