...
import com.netflix.spinnaker.igor.history.model.HelmOciDockerEvent
...
class HelmOciMonitor extends CommonPollingMonitor<ImageDelta, DockerPollingDelta> {
  ...
  HelmOciMonitor(IgorConfigurationProperties properties,
                 ...
                 HelmOciDockerRegistryCache cache,
  ...
  DockerPollingDelta generateDelta(PollContext ctx) {
    ...
    List<TaggedImage> images = AuthenticatedRequest.allowAnonymous { dockerRegistryAccounts.service.getChartsByAccount(account, true) }
  ...
  void postEvent(Set<String> cachedImagesForAccount, TaggedImage image, String imageId) {
    ...
    GenericArtifact dockerArtifact = new GenericArtifact("helm/image", image.repository, image.tag, "${image.registry}/${image.repository}:${image.tag}")
    dockerArtifact.metadata = [registry: image.registry]

    AuthenticatedRequest.allowAnonymous {
      echoService.get().postEvent(new HelmOciDockerEvent(content: new HelmOciDockerEvent.Content(
