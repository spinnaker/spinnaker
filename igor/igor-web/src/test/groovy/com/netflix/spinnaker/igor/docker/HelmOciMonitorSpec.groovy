...
class HelmOciMonitorSpec extends Specification {
  ...
  def helmOciDockerRegistryCache = Mock(HelmOciDockerRegistryCache)
  def dockerRegistryAccounts = Mock(DockerRegistryAccounts)
  def echoService = Mock(EchoService)
  def keelService = Mock(KeelService)
  def helmOciDockerRegistryProperties = new HelmOciDockerRegistryProperties(enabled: true, itemUpperThreshold: 5)
  ...
  void 'should include decorated artifact in the payload'() {
    ...
    1 * echoService.postEvent({ DockerEvent event ->
      ...
      assert event.artifact.type == "helm/image"
  ...
  void "should update cache if image is not already cached"() {
    ...
    then:
    helmOciDockerRegistryCache.getLastDigest(_, _, _) >> cachedDigest
  ...
  private HelmOciMonitor createSubject() {
    return new HelmOciMonitor(properties, registry, dynamicConfig, discoveryStatusListener, lockService, helmOciDockerRegistryCache, dockerRegistryAccounts, Optional.of(echoService), Optional.of(keelService), helmOciDockerRegistryProperties, Mock(TaskScheduler))
  }

  private static String keyFromTaggedImage(TaggedImage taggedImage) {
    return new DockerRegistryV2Key("prefix", HelmOciDockerRegistryCache.ID, taggedImage.account, taggedImage.registry, taggedImage.tag).toString()
  }
}
