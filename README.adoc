...
This repository is the Spinnaker monorepo: it holds the source for every core microservice
(`clouddriver`, `deck`, `echo`, `fiat`, `front50`, `gate`, `igor`, `kayenta`, `keel`, `kork`,
`orca`, `rosco`, `spin`) and is where builds, releases, and issue tracking all happen. See
link:MONOREPO.md[MONOREPO.md] for how the monorepo is structured and built. Please see the
following for more information about spinnaker and its state:
...
This repository is the monorepo containing the source for every core Spinnaker microservice,
alongside issue tracking for the project as a whole.
