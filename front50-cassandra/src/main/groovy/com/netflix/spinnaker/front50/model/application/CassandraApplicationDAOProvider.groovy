...
class CassandraApplicationDAOProvider implements ApplicationDAOProvider<CassandraCredentials> {
  ...
  boolean supports(Class<?> credentialsClass) {
    return CassandraCredentials.isAssignableFrom(credentialsClass)
  }

  @Override
  ApplicationDAO getForAccount(CassandraCredentials credentials) {
    return cassandraApplicationDAO
  }
}