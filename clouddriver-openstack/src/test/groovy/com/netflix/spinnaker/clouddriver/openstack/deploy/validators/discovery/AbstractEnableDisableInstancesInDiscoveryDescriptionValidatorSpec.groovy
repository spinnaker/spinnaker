...
package com.netflix.spinnaker.clouddriver.openstack.deploy.validators.discovery
...
@Unroll
class AbstractEnableDisableInstancesInDiscoveryDescriptionValidatorSpec extends Specification {
  ...
  AbstractEnableDisableInstancesInDiscoveryDescriptionValidator validator
  ...
  def "Validate no exception - #type"() {
    given:
    validator = type.newInstance(accountCredentialsProvider: provider)
    ...
    where:
    type << [EnableInstancesInDiscoveryDescriptionValidator, DisableInstancesInDiscoveryDescriptionValidator]
  }

  def "Validate empty account exception - #type"() {
    given:
    validator = type.newInstance(accountCredentialsProvider: provider)
  ...
  def "Validate empty instance list exception - #type"() {
    given:
    validator = type.newInstance(accountCredentialsProvider: provider)
