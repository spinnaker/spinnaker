...
class DisableOpenstackAtomicOperationSpec extends Specification {
  ...
  def "disable stack adds instances to load balancer"() {
    given:
    ...
    pools.each {
      ids.each {
        ...
        1 * provider.getMemberIdForInstance(description.region, _, _)
        1 * provider.removeMemberFromLoadBalancerPool(description.region, _)
      }
    }
    noExceptionThrown()
  }
