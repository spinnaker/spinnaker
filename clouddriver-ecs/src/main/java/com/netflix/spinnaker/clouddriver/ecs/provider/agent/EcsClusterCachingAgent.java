...
public class EcsClusterCachingAgent extends AbstractEcsCachingAgent<String> {
  private static final Collection<AgentDataType> types =
      Collections.unmodifiableCollection(
          Arrays.asList(AUTHORITATIVE.forType(ECS_CLUSTERS.toString())));
  ...
  protected Map<String, Collection<CacheData>> generateFreshData(Collection<String> clusterArns) {
    ...
    for (String clusterArn : clusterArns) {
      ...
      Map<String, Object> attributes =
          convertClusterArnToAttributes(accountName, region, clusterArn);
  ...
  public static Map<String, Object> convertClusterArnToAttributes(
      String accountName, String region, String clusterArn) {
    String clusterName = StringUtils.substringAfterLast(clusterArn, "/");
