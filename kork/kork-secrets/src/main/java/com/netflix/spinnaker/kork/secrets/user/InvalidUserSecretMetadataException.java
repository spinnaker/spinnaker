...
public class InvalidUserSecretMetadataException extends InvalidSecretFormatException
  ...
  public InvalidUserSecretMetadataException(SecretReference secretReference, Throwable cause) {
    super(
        String.format(
            "%s in %s",
            SecretErrorCode.INVALID_USER_SECRET_METADATA.getMessage(), secretReference.getUri()),
        cause);
