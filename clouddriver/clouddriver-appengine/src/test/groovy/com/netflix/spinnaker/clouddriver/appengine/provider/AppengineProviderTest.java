...
final class AppengineProviderTest {
   ...
   * full eviction, and every AppEngine caching agent's always-present-key CacheResult is silently
   * discarded before its eviction diff can run. A regression here would reintroduce the stale-cache
   * bug for every AppEngine account on the SQL cache backend.
   */
  @Test
  void supportsFullEviction() {
    AppengineProvider provider = new AppengineProvider(new AppengineCloudProvider());
