...
class TerminateAzureInstancesAtomicOperationSpec extends Specification {
  ...
  void "terminate reimages rather than deleting, so capacity is untouched"() {
    when:
    new TerminateAzureInstancesAtomicOperation(description()).operate([])

    then:
    1 * computeClient.reimageInstances("myapp-westus", SERVER_GROUP, ["${SERVER_GROUP}_3"])
    0 * computeClient.deleteInstances(_, _, _)
  }
