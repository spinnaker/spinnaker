...
public class StateMachine {
  ...
  public AgentState getAgent(String agentType) {
    try (var result = jooq.selectFrom(table(PUBSUB_AGENT_STATE)).where(field(AGENT_TYPE).eq(agentType))) {
  ...
  public void acquireLock(Agent agent, long timeOutMs, State stateToApply) throws Exception {
    @NotNull ResultSet lockRecord = jooq.selectFrom(table(PUBSUB_AGENT_STATE)).where(field(AGENT_TYPE).eq(agent.getAgentType())).forUpdate().queryTimeout((int) (timeOutMs / 1000)).fetchResultSet();
    if (lockRecord.next()) {
  ...
  public int changeStateUnlessMarkedForDeletion(Agent agent, State stateToSet) {
    return jooq
      .update(table(PUBSUB_AGENT_STATE)).
      set(field("current_state"), stateToSet)
      .where(AGENT_TYPE, agent.getAgentType())
      .andNot(field("current_state").eq(StateMachine.State.DELETED.name())).execute();
  }
  ...
  /**
   * Delete the actual agent from the tracking table.  This should ONLY occur AFTER a few hours as some agents are "rescheduled" aka deleted & recreated via an unschedule/reschedule operation.
   *
   * @param agentType
   */
  public void delete(String agentType) {
    // we keep the agent IN the table marked as "DELETED" so that we can on agent COMPLETION (if already running) know that it's marked for deletion, and skip writing data, just let it complete
    int rowsDeleted = jooq.deleteFrom(table(PUBSUB_AGENT_STATE)).where(field(AGENT_TYPE).eq(agentType)).execute();
    if (rowsDeleted != 1) {
      log.error("Failed to actually delete agent {} from pubsub_agent_state as deleted.  YOU MAY Have dangling agent data as a result in the pubsub_agent_state table.", agentType);
    }
  }
  ...
  public void createOrUpdateAgent(Agent agent, State stateToSet) throws Exception {
    ...
    try {
      ...
      if (lockRecord.next()) {
        // already exists... so we probably are rescheduling an agent.  Some of the providers to an unschedule/reschedule to reschedule various agents.
    ...
    public static class AgentState {
      ...
      public static AgentState from(Agent agent) {
        return builder().agentType(agent.getAgentType())
          .currentState(State.NOT_STARTED)
          .previousState(null)
          .lastTransitionTime(System.currentTimeMillis())
          .lastExecutionTime(0)
          .lastDuration(0)
          .dataProcessed(0).build();
      }
    }
