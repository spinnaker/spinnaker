...
public class PubSubAgentRunner implements MessageListener {
  ...
  public void onMessage(Message message, byte[] pattern) {
    ...
    long lastExecutionTimeWithBuffer = (long) (stateMachine.getAgent(agentType).getLastExecutionTime() * percentMaxOverNormalDuration);
    ...
    try {
      ...
      if (agent != null) {
        ...
        if (agent instanceof CachingAgent.CacheExecution cacheExecutionAgent) {
          List<Tag> tags = List.of(Tag.of("agentType", agentType), Tag.of("state", StateMachine.State.FINISHED.name()));
          ...
          meterRegistry.gauge("cats.pubsub.execution.size", tags, cacheResult.getCacheResults().size());

          timer = new StopWatch();
          timer.start();
          cacheExecutionAgent.storeAgentResult(agent, cacheResult);
          timer.stop();
          meterRegistry.gauge("cats.pubsub.execution.storagetime", tags, timer.getTotalTimeMillis());
          meterRegistry.counter("cats.pubsub.agents.processed", List.of(Tag.of("agentType", agentType), Tag.of("state", StateMachine.State.FINISHED.name()))).increment();
        } else {
