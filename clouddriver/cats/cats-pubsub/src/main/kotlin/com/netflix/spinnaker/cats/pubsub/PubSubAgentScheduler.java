 ...
 * <p>
 * There are three key compoentns to this scheduler:
 * 1) A state management machine that tracks known agents and their current processing state.  This knows ALL known agents
 * and their state.  It also tracks the last time an agent was run, and the last time it was scheduled to run.
 * 3) A poller that looks up the state and queues agents to be run based upon state/status
 * 4) A message handler that given an agent identifed by "agentType", will actually do the agent execution.  This will ALSO
 * update the state.
 * <p>
...
@Log4j2
public class PubSubAgentScheduler extends CatsModuleAware implements Runnable, AgentScheduler<AgentLock> {

  public static final Set<StateMachine.State> QUEABLE_STATE_LIST = Set.of(StateMachine.State.FINISHED, StateMachine.State.NOT_STARTED, StateMachine.State.FAILED);
  ...
  @Autowired
  PubSubAgentScheduler(ProviderRegistry providerRegistry, StateMachine stateMachine, AgentIntervalProvider intervalProvider, RedisTemplate<String, String> redisTemplate) {
...
// used in ondemand to say "OK need to run an on demand agent,"
  ...
  public void run() {
    ...
    stateMachine.listAgents().stream().filter(agent -> agent.getCurrentState() == StateMachine.State.RUNNING).forEach(runningAgent -> {
      Agent agentThatCouldBeRun = providerRegistry.getAgentForProviderName(runningAgent.getAgentType());
      ...
      if (interval.getTimeout() <= System.currentTimeMillis() - runningAgent.getLastTransitionTime()) {
        ...
        try {
          // ONLY send to the queue IF we successfully moved it to a pending state.  This skips over a record marked for deletion.  TECHNICALLY this shouldn't happen.
          if (stateMachine.changeStateUnlessMarkedForDeletion(agentThatCouldBeRun, StateMachine.State.PENDING) >= 0) {
