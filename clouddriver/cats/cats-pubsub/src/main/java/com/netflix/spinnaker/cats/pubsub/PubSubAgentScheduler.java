...
public class PubSubAgentScheduler extends CatsModuleAware
  ...
  public void run() {
    ...
    // Find any agents still running after their max interval (if one available) and reschedule them.  They likely failed
    // or were hung.
    stateMachine
      ...
      .forEach(
        runningAgent -> {
          ...
          if (interval.getTimeout()
            ...
            // for processing.
            try {
              log.warn("Found agent {} that is in a running state for a longer than it should have been ({}).  Rescheduling.  WARNING:  THe agent MAY have been running before hand... and not finished.  Check agent for more information/logs and duration data!",
              ...
              e.printStackTrace();
              log.error("Failed to send agent {} for processing OR update state!  ", runningAgent.getAgentType(), e);
            }
          }
        });
      ...
      .forEach(
        agent -> {
          ...
          if (agentThatCouldBeRun == null) {
            log.warn("Can't find agent " + agent.getAgentType() + " in the list of available provider agents!  Will skip scheduling and try on next run.  IF you see this a lot theres probably a bug someplace on agent lists/registry behavior...");
            return;
          }
          AgentIntervalProvider.Interval interval = intervalProvider.getInterval(agentThatCouldBeRun);
          log.debug("Looking at agent {} that COULD be scheduled.  The interval between runs is {} and the last time run was {} seconds ago ",
            agent.getAgentType(),
            interval.getInterval(),
            ((System.currentTimeMillis() - agent.getLastExecutionTime() - interval.getInterval())/ 1000));
          if (agent.getLastExecutionTime()
            ...
            try {
              ...
              stateMachine.createOrUpdateAgent(agentThatCouldBeRun.getAgentType(), StateMachine.State.PENDING);
    ...
    // If it's been in pending for longer than 20 minutes... probably an issue with the queue.  re-enqueue it with
    // STRONG warnings
    stateMachine
      .listAgentsFilteredWhereIn(Set.of(StateMachine.State.PENDING))
      .forEach(
        runningAgent -> {
          // It's been pending for longer than 20 minutes... probably an issue with the queue.  re-enqueue it with
          if (runningAgent.getLastTransitionTime() < System.currentTimeMillis() -  20 *60 * 1000) {
            // The agent has been running TOO long.  This is LIKELY a bad situation.  Figure out
            // which pod it is, kill the agent process, and move on.  UNFORTUNATELY there's no
            // "interrupt" option on Agents in spinnaker to DO This :( NOT Yet :(
            stateMachine.createOrUpdateAgent(runningAgent.getAgentType(), StateMachine.State.PENDING);
            redisTemplate.convertAndSend(CHANNEL, runningAgent.getAgentType());
          }
        });
