...
final class CloudrunProviderTest {
   ...
   * full eviction, and every CloudRun caching agent's always-present-key CacheResult is silently
   * discarded before its eviction diff can run. A regression here would reintroduce the stale-cache
   * bug for every CloudRun account on the SQL cache backend.
   */
  @Test
  void supportsFullEviction() {
    CloudrunProvider provider = new CloudrunProvider(new CloudrunCloudProvider());
