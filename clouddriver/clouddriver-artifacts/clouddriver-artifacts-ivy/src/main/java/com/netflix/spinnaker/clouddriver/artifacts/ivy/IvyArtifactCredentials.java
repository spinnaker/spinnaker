...
@NonnullByDefault
@Slf4j
public class IvyArtifactCredentials extends AbstractArtifactCredentials {
  ...
  public InputStream download(Artifact artifact) {
    ...
    if (parts.length < 3) {
      throw new IllegalArgumentException(
          "Ivy artifact reference must have a group, artifact, and version separated by ':'");
    }
    ...
    try {
      ...
      return Arrays.stream(report.getAllArtifactsReports())
          ...
          .map(
              rep -> {
                try {
                  return new DiskFreeingInputStream(
                      new FileInputStream(rep.getLocalFile()), cacheDir);
          ...
          .orElseThrow(
              () ->
                  new IllegalArgumentException(
                      "Unable to resolve artifact for reference '"
