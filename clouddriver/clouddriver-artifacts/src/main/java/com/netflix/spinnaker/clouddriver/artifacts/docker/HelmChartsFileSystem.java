...
@Slf4j
public class HelmChartsFileSystem {

  private static final Path CLONES_HOME =
      Paths.get(System.getProperty("java.io.tmpdir"), "helmcharts");

  private final HelmChartArtifactProviderProperties config;
  private final Map<String, Lock> pathLocks = new ConcurrentHashMap<>();

  public HelmChartsFileSystem(HelmChartArtifactProviderProperties properties) {
  ...
  @Scheduled(
      fixedDelayString =
          "${artifacts.helm-oci.clone-retention-check-ms:"
              + HelmChartArtifactProviderProperties.DEFAULT_CLONE_RETENTION_CHECK_MS
              + "}")
