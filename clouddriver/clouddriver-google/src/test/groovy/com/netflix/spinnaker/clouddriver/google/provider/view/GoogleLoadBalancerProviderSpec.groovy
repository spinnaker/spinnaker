...
class GoogleLoadBalancerProviderSpec extends Specification {
  ...
  void "should report the port range for a regional external network load balancer without discrete ports"() {
    setup:
      def cacheView = Mock(Cache)
      def serverGroup = Mock(CacheData)
      def serverGroupRelationships = [:]
    ...
    then:
      ...
      1 * serverGroup.getRelationships() >> serverGroupRelationships
      details.size() == 1
      details[0].listenerDescriptions[0].listener.loadBalancerPort == "8080-8090"
      details[0].listenerDescriptions[0].listener.instancePort == "8080-8090"
  }

  void "should describe an internal load balancer that reports no discrete ports"() {
    setup:
      def cacheView = Mock(Cache)
      def serverGroup = Mock(CacheData)
      def serverGroupRelationships = [:]
      serverGroupRelationships.put(LOAD_BALANCERS.ns, ["internal_lb_identifier"])
      ...
      def googleLoadBalancerView = Mock(CacheData)
      def attributes = [
        type               : GoogleLoadBalancerType.INTERNAL,
        loadBalancingScheme: GoogleLoadBalancingScheme.INTERNAL,
        name               : LOAD_BALANCER_NAME,
        account            : ACCOUNT_NAME,
        region             : REGION_EUROPE,
        ipAddress          : "10.0.0.5",
        ipProtocol         : "TCP",
        backendService     : [name: "internal-backend"],
        healths            : []
      ]
      googleLoadBalancerView.getAttributes() >> attributes
      googleLoadBalancerView.getRelationships() >> loadBalancerRelations
    ...
    then:
      noExceptionThrown()
      _ * cacheView.filterIdentifiers(LOAD_BALANCERS.ns, _) >> ["internal_lb_identifier"]
      1 * cacheView.getAll(LOAD_BALANCERS.ns, _, _) >> [googleLoadBalancerView]
      1 * cacheView.filterIdentifiers(SERVER_GROUPS.ns, _) >> SERVER_GROUP_IDS
      1 * cacheView.getAll(SERVER_GROUPS.ns, SERVER_GROUP_IDS) >> [serverGroup]
      1 * serverGroup.getRelationships() >> serverGroupRelationships
      details.size() == 1
      details[0].listenerDescriptions[0].listener.loadBalancerPort == null
  }
