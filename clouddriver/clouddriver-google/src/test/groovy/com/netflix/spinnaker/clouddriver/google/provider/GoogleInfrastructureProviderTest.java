...
final class GoogleInfrastructureProviderTest {
   ...
   * full eviction, and every Google caching agent's CacheResultBuilder-driven empty-cache backfill
   * is silently discarded before its eviction diff can run. A regression here would reintroduce the
   * stale-cache bug for every Google account on the SQL cache backend.
   */
  @Test
  void supportsFullEviction() {
    GoogleInfrastructureProvider provider = new GoogleInfrastructureProvider();
