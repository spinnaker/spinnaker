...
public abstract class AbstractUpsertGoogleRegionalHttpLoadBalancerAtomicOperation
  ...
  private static final Logger log =
      LoggerFactory.getLogger(AbstractUpsertGoogleRegionalHttpLoadBalancerAtomicOperation.class);
  ...
  protected AbstractUpsertGoogleRegionalHttpLoadBalancerAtomicOperation(
      UpsertGoogleLoadBalancerDescription description) {
    this.description = description;
  }

  protected abstract String getBasePhase();

  protected abstract String getLoadBalancerDescriptionLabel();
    ...
    try {
        ...
        existingServices.stream()
            ...
            .ifPresent(
                existingService -> {
                  ...
                  Boolean differentProtocol =
                      backendProtocolNeedsUpdate(existingService, backendService);
                  Integer drainingSec =
      ...
      for (GoogleBackendService backendService : backendServicesFromDescription) {
        ...
        if (!serviceExistsSet.contains(backendService.getName())) {
          ...
          configureBackendProtocol(service, backendService);
          service.setPortName(
          ...
          if (serviceNeedsUpdatedSet.contains(backendService.getName())) {
            ...
            bsToUpdate.setPortName(
                backendService.getPortName() != null
                    ? backendService.getPortName()
                    : GoogleHttpLoadBalancingPolicy.HTTP_DEFAULT_PORT_NAME);
            configureBackendProtocol(bsToUpdate, backendService);
  ...
  protected Operation deleteRegionalListenerIfOwned(
      ...
      throws IOException {
    if (!validateRegionalListenerOwnership(
        compute, project, region, forwardingRuleName, expectedUrlMapName)) {
      return null;
    }
  ...
  private boolean validateRegionalListenerOwnership(
      Compute compute,
      String project,
      String region,
      String forwardingRuleName,
  ...
  protected void validateImmutableListener(
    ...
    String desiredPort =
        StringGroovyMethods.asBoolean(desired.getCertificate())
            ? "443"
            : normalizeSinglePort(desired.getPortRange());
    rejectImmutableListenerChange(
        existingRule, "portRange", !Objects.equals(existingPort, desiredPort));
    rejectImmutableListenerChange(
        existingRule,
        "ipAddress",
        StringUtils.isNotBlank(desired.getIpAddress())
            && !Objects.equals(existingRule.getIPAddress(), desired.getIpAddress()));
    rejectImmutableListenerChange(
        existingRule,
        "networkTier",
        StringUtils.isNotBlank(description.getNetworkTier())
            && !Objects.equals(existingRule.getNetworkTier(), description.getNetworkTier()));

    GoogleTargetProxyType existingProxyType = Utils.getTargetProxyType(existingRule.getTarget());
    GoogleTargetProxyType desiredProxyType =
        StringGroovyMethods.asBoolean(desired.getCertificate())
            ? GoogleTargetProxyType.HTTPS
            : GoogleTargetProxyType.HTTP;
    rejectImmutableListenerChange(existingRule, "proxyType", existingProxyType != desiredProxyType);
  }

  private void rejectImmutableListenerChange(
      ForwardingRule existingRule, String field, boolean changed) {
    if (changed) {
      throw new GoogleOperationException(
          "Immutable listener field "
              + field
              + " cannot change for same-name listener "
              + existingRule.getName()
              + "; rename or recreate the listener.");
    }
  }
  ...
  private static String normalizeSinglePort(String portRange) {
    ...
    String[] bounds = portRange.split("-", -1);
    return bounds.length == 2 && bounds[0].equals(bounds[1]) ? bounds[0] : portRange;
  }

  private static String normalizeBackendProtocol(String protocol) {
    return StringUtils.isBlank(protocol) ? "HTTP" : protocol;
  }

  protected boolean backendProtocolNeedsUpdate(
      BackendService existingService, GoogleBackendService desiredService) {
    return managesBackendProtocol()
        && !Objects.equals(
            normalizeBackendProtocol(existingService.getProtocol()),
            normalizeBackendProtocol(desiredService.getProtocol()));
  }

  protected void configureBackendProtocol(
      BackendService targetService, GoogleBackendService desiredService) {
    if (managesBackendProtocol()) {
      targetService.setProtocol(normalizeBackendProtocol(desiredService.getProtocol()));
    }
  }
  ...
  public void setGoogleOperationPoller(GoogleOperationPoller googleOperationPoller) {
