...
final class CloudFoundryProviderTest {
   ...
   * full eviction, and every CloudFoundry caching agent's always-present-key CacheResult is
   * silently discarded before its eviction diff can run. A regression here would reintroduce the
   * stale-cache bug for every CloudFoundry account on the SQL cache backend.
   */
  @Test
