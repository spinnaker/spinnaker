...
public class KubernetesCronJobHandler extends KubernetesHandler
  ...
  public Status status(KubernetesManifest manifest) {
    V1CronJob v1CronJob = KubernetesCacheDataConverter.getResource(manifest, V1CronJob.class);
    return status(v1CronJob);
  }
  ...
  private Status status(V1CronJob job) {
    V1CronJobStatus status = job.getStatus();
    if (status == null) {
