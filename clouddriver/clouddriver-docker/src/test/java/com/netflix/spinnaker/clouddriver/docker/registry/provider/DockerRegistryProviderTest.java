...
final class DockerRegistryProviderTest {
   ...
   * full eviction, and every Docker Registry caching agent's always-present-key CacheResult is
   * silently discarded before its eviction diff can run. A regression here would reintroduce the
   * stale-cache bug for every Docker Registry account on the SQL cache backend.
   */
  @Test
  void supportsFullEviction() {
    DockerRegistryProvider provider = new DockerRegistryProvider(new DockerRegistryCloudProvider());
