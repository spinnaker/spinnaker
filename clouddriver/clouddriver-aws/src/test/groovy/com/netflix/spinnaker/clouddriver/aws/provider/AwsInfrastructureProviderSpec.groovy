...
class AwsInfrastructureProviderSpec extends Specification {
  ...
  // disables full eviction, and every EC2 infrastructure caching agent's always-present-key
  // CacheResult is silently discarded before its eviction diff can run. A regression here would
  // reintroduce the stale-cache bug for every EC2 account on the SQL cache backend.
  def "supports full eviction"() {
    given:
    def provider = new AwsInfrastructureProvider()
