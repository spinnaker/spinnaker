...
class UpsertAlarmDescriptionValidator extends AmazonDescriptionValidationSupport<UpsertAlarmDescription> {
  ...
  void validate(List priorDescriptions, UpsertAlarmDescription description, ValidationErrors errors) {
    ...
    if (!description.metricName) {
      rejectNull "metricName", errors
    }

    if (!description.namespace) {
      rejectNull "namespace", errors
    }

    if (!description.comparisonOperator) {
      rejectNull "comparisonOperator", errors
    }

    if (!description.evaluationPeriods) {
      rejectNull "evaluationPeriods", errors
    }
    ...
    if (!description.statistic) {
      rejectNull "statistic", errors
    }
