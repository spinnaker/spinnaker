...
public class GoogleEditPublisherCommand extends AbstractEditPublisherCommand<GooglePublisher> {
  ...
  protected Publisher editPublisher(GooglePublisher publisher) {
    publisher.setJsonPath(isSet(jsonPath) ? jsonPath : publisher.getJsonPath());
    publisher.setProject(isSet(project) ? project : publisher.getProject());
    publisher.setTopicName(isSet(topicName) ? topicName : publisher.getTopicName());
