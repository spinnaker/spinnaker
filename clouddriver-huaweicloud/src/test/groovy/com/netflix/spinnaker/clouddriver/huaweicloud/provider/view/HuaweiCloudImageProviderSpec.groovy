...
class HuaweiCloudImageProviderSpec extends Specification {

  @Subject
  HuaweiCloudImageProvider provider

  WriteableCache cache = new InMemoryCache()
  ObjectMapper mapper = new ObjectMapper()

  def setup() {
    provider = new HuaweiCloudImageProvider(cache, mapper)
    cache.mergeAll(Keys.Namespace.IMAGES.ns, getAllImages())
  }

  void "getAll lists all and does not choke on deserializing routingConfig"() {
    when:
      def result = provider.getAll("", "")
      def result1 = provider.getAll('global', 'cn-north-1')
  ...
  private List<CacheData> getAllImages() {
    imageList.collect { Image image ->
      String cacheId = Keys.getImageKey(image.id, 'global', 'cn-north-1')
