...
interface ResourceHandler<T : Any> : KeelPlugin {
  ...
  val validators: List<ResourceValidator<*>>
   ...
   *  custom validation should be done with a [ResourceValidator].
   *
  ...
  fun validate(resource: Resource<Any>): Resource<T> {
    ...
    for (validator in validators) {
      if (validator.handles(resource.apiVersion, resource.kind)) {
        log.debug("Validating ${resource.metadata.name} with ${validator.javaClass}")
        validatedResource = validator.validate(validatedResource) as Resource<T>
      }
    }
