...
public interface EntityTagsProvider {
  /**
   * Fetch EntityTags by any combination of {@code cloudProvider}/{@code type}/{@code
   * idPrefix}/{@code tags}
   */
  Collection<EntityTags> getAll(
  ...
  /**
   * Remove all entity tags within a particular namespace, optionally deleting from the source of
   * truth (front50).
   */
  Map<String, Object> deleteByNamespace(String namespace, boolean dryRun, boolean deleteFromSource);

  /**
   * Remove all entity tags with a particular tag, optionally deleting from the source of truth
   * (front50).
   */
  Map<String, Object> deleteByTag(String tag, boolean dryRun, boolean deleteFromSource);
