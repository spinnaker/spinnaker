...
/**
 * A loadBalancerProvider is an interface for the application to retrieve {@link LoadBalancer}
 * objects. The interface provides a common contract for which one or many providers can be queried
 * for their knowledge of load balancers at a given depth of specificity.
 *
 * <p>This is a temporary class for consolidating the load balancer controllers for each cloud
 * provider. Each cloud provider-specific controller will implement this interface (it already does
...
public interface LoadBalancerProvider<T extends LoadBalancer> {
   ...
   * the load balancer name follows the Frigga naming conventions for load balancers (i.e., the load
   * balancer name starts with the application name, followed by a hyphen) - the load balancer is
   * used by a server group in the application
   *
   * @param application the name of the application
   * @return a collection of load balancers with all attributes populated and a minimal amount of
   *     data for each server group: its name, region, and *only* the instances attached to the load
   *     balancers described above. The instances will have a minimal amount of data, as well: name,
   *     zone, and health related to any load balancers
   */
  @Empty
