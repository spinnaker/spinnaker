...
func TestNewGateClientSetsContextAccessTokenFromOAuth2CachedToken(t *testing.T) {
	...
	select {
	case authHeader := <-loginHeaders:
		if authHeader != "Bearer "+token {
			t.Fatalf("unexpected Authorization header on login request: got %q, want %q", authHeader, "Bearer "+token)
		}
	default:
		t.Fatal("expected oauth2 login endpoint to be called")
	}
}
