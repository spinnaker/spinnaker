...
describe('Directive: Titus SG Advanced Settings Selector', function() {
  beforeEach(window.module(require('./serverGroupAdvancedSettingsSelector.directive.js').name));

  beforeEach(
    window.inject(function($rootScope, $compile) {
      this.scope = $rootScope.$new();
      this.scope.command = { instanceMetadata: [], tags: [] };
      this.elem = angular.element('<titus-server-group-advanced-settings-selector command="command" />');
  ...
  it('should correctly remove a metadata key/value from the command', function() {
    this.scope.command.instanceMetadata.push({ key: 'myKey1', value: 'myVal1' }, { key: 'myKey2', value: 'myVal2' });
    this.scope.$apply();
