...
describe('Controller: pipelineTrigger', function() {

  beforeEach(module('spinnaker.pipelines.trigger.pipeline'));
  ...
  beforeEach(inject(function ($controller, $rootScope, $q, igorService) {
    ...
    this.initializeController = function (trigger) {
      this.controller = $controller('pipelineTriggerCtrl', {
        $scope: this.$scope,
        trigger: trigger,
  ...
  describe('updateJobsList', function() {
    it('gets list of jobs when initialized with a trigger with a master and sets loading states', function() {
      var $q = this.$q,
          ...
          trigger = {master: 'pipeline', job: 'some_job'};

      spyOn(this.igorService, 'listJobsForMaster').and.returnValue($q.when(jobs));
      spyOn(this.igorService, 'listMasters').and.returnValue($q.when(['pipeline']));
      ...
      expect($scope.masters).toEqual(['pipeline']);
