...
module.exports = angular
  .module('spinnaker.titan.validation.applicationName', [
    require('../../core/application/modal/validation/applicationName.validator.js'),
  ])
  .factory('titanApplicationNameValidator', function () {
    ...
    function validateLength(name, warnings, errors) {
      if (name.length > 250) {
        errors.push('The maximum length for an application in Titan is 250 characters.');
        return;
      }
      if (name.length > 240) {
        if (name.length >= 248) {
  ...
  .run(function(applicationNameValidator, titanApplicationNameValidator) {
    applicationNameValidator.registerValidator('titan', titanApplicationNameValidator);
  });
