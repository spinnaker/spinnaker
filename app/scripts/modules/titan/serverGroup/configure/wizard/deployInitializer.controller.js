...
module.exports = angular.module('spinnaker.serverGroup.configure.titan.deployInitialization.controller', [
  ...
  .controller('titanDeployInitializerCtrl', function($scope, titanServerGroupCommandBuilder, serverGroupReader, _) {
    var controller = this;
    ...
    function buildEmptyCommand() {
      return titanServerGroupCommandBuilder.buildNewServerGroupCommand($scope.application, {mode: 'createPipeline'}).then(function(command) {
    ...
    function buildCommandFromTemplate(serverGroup) {
      return serverGroupReader.getServerGroup($scope.application.name, serverGroup.account, serverGroup.region, serverGroup.name).then(function (details) {
        ...
        return titanServerGroupCommandBuilder.buildServerGroupCommandFromExisting($scope.application, details, 'editPipeline').then(function (command) {
          applyCommandToScope(command);
        });
      });
    }
