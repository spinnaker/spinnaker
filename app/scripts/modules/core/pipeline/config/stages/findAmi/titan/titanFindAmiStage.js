...
//BEN_TODO
module.exports = angular.module('spinnaker.core.pipeline.stage.titan.findAmiStage', [
  ...
  .config(function(pipelineConfigProvider) {
  ...
  }).controller('titanFindAmiStageCtrl', function($scope, accountService) {
    ...
    accountService.listAccounts('titan').then(function (accounts) {
    ...
    if (!stage.credentials && $scope.application.defaultCredentials.titan) {
      stage.credentials = $scope.application.defaultCredentials.titan;
    }
    if (!stage.regions.length && $scope.application.defaultRegions.titan) {
      stage.regions.push($scope.application.defaultRegions.titan);
    }
