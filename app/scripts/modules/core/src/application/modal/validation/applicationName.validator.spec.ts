...
describe('Validator: applicationName', () => {
  ...
  beforeEach(mock.module(APPLICATION_NAME_VALIDATOR, EXAMPLE_APPLICATION_NAME_VALIDATOR));
  ...
  describe('warning messages', () => {
    it('aggregates warning messages when multiple or no providers are specified', () => {
      ...
      validator.validate(validator1.COMMON_WARNING_CONDITION, []).then(r => (result = r));
      $scope.$digest();
      ...
      validator
        .validate(validator1.COMMON_WARNING_CONDITION, [validator1.provider, validator2.provider])
        .then(r => (result = r));
      $scope.$digest();
      expect(result.warnings[0].message).toEqual(validator1.COMMON_WARNING_MESSAGE);
      expect(result.warnings[0].cloudProvider).toEqual(validator1.provider);
    ...
    it('provides warnings only from provider when specified', () => {
      let result: IApplicationNameValidationResult = null;
      validator.validate(validator1.WARNING_CONDITION, [validator2.provider]).then(r => (result = r));
      $scope.$digest();
      expect(result.warnings.length).toBe(0);

      validator.validate(validator1.WARNING_CONDITION, [validator1.provider]).then(r => (result = r));
      $scope.$digest();
  ...
  describe('error messages', () => {
    it('aggregates error messages when multiple or no providers are specified', () => {
      let result: IApplicationNameValidationResult = null;
      validator.validate(validator1.COMMON_ERROR_CONDITION, []).then(r => (result = r));
      $scope.$digest();
      ...
      validator
        .validate(validator1.COMMON_ERROR_CONDITION, [validator1.provider, validator2.provider])
        .then(r => (result = r));
      $scope.$digest();
      expect(result.errors[0].message).toEqual(validator1.COMMON_ERROR_MESSAGE);
      expect(result.errors[0].cloudProvider).toEqual(validator1.provider);
    ...
    it('provides errors only from provider when specified', () => {
      let result: IApplicationNameValidationResult = null;
      validator.validate(validator1.ERROR_CONDITION, [validator2.provider]).then(r => (result = r));
      $scope.$digest();
      expect(result.errors.length).toBe(0);

      validator.validate(validator1.ERROR_CONDITION, [validator1.provider]).then(r => (result = r));
      $scope.$digest();
