...
export class SecurityGroupTransformerService {
  constructor(private providerServiceDelegate: ProviderServiceDelegate, private accountService: AccountService) { 'ngInject'; }

  public normalizeSecurityGroup(securityGroup: ISecurityGroup): IPromise<ISecurityGroup> {
    return this.accountService.getAccountDetails(securityGroup.account)
      .then((accountDetails: IAccountDetails) => {
...
module(SECURITY_GROUP_TRANSFORMER_SERVICE, [ ACCOUNT_SERVICE, PROVIDER_SERVICE_DELEGATE ])
