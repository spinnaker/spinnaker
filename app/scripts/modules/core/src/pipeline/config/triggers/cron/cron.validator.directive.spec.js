...
describe('Validator: cronExpression', function() {
  ...
  beforeEach(window.module(require('./cron.validator.directive').name));
  ...
  beforeEach(
    window.inject(function($rootScope, _$compile_, _cronValidationService_, _$q_) {
      scope = $rootScope.$new();
      $compile = _$compile_;
  ...
  beforeEach(function() {
    this.initialize = function(val) {
      ...
      var input =
        '<input type="text" name="cron" ng-model="command.expression" cron-validator cron-validation-messages="validationMessages"/>';
  ...
  describe('successful validation', function() {
    it('converts description to lower case and adds to scope', function() {
      spyOn(cronValidationService, 'validate').and.returnValue(
        $q.when({ valid: true, description: 'Every day at 10:30 AM' }),
      );
