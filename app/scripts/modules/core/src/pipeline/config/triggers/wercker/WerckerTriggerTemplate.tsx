...
export interface IWerckerTriggerTemplateState {
  builds?: IBuild[];
...
export class WerckerTriggerTemplate extends React.Component<
  ITriggerTemplateComponentProps,
  IWerckerTriggerTemplateState
> {
  public static formatLabel(trigger: IBuildTrigger): IPromise<string> {
    return $q.when(`(Wercker) ${trigger.master}: ${trigger.job}`);
  }
  ...
  private buildLoadSuccess = (allBuilds: IBuild[]) => {
    const newState: Partial<IWerckerTriggerTemplateState> = {
      buildsLoading: false,
    };
  ...
  private initialize = () => {
    ...
    if (trigger.type !== BuildServiceType.Wercker) {
      return;
    }
