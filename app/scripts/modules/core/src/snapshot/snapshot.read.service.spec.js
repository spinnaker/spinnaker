...
describe('Service: SnapshotRead', function() {
  ...
  beforeEach(window.module(require('./snapshot.read.service.js').name));

  beforeEach(
    window.inject(function($httpBackend, snapshotReader) {
  ...
  describe('getSnapshotHistory', function() {
    ...
    it('returns what the endpoint returns', function() {
      ...
      this.snapshotReader
        .getSnapshotHistory(application, account, params)
        .then(snapshots => {
    ...
    it('does not fail if not passed parameters', function() {
      this.$httpBackend
        ...
        .respond([]);
      try {
        this.snapshotReader.getSnapshotHistory(application, account).catch(e => {
