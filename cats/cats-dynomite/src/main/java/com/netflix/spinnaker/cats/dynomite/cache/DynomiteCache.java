...
public class DynomiteCache extends AbstractRedisCache {
  ...
  public DynomiteCache(
    ...
    this.compressionStrategy =
        compressionStrategy == null ? new NoopCompression() : compressionStrategy;
  }
  ...
  public void mergeItems(String type, Collection<CacheData> items) {
    ...
    Failsafe.with(REDIS_RETRY_POLICY)
        .onRetriesExceeded(
            failure -> {
              log.error(
                  "Encountered repeated failures while caching {}:{}, attempting cleanup",
                  prefix,
                  type,
                  failure);
              try {
              ...
              throw new ExcessiveDynoFailureRetries(
                  format("Running cache agent %s:%s", prefix, type), failure);
            })
        .run(
            () ->
                redisClientDelegate.withPipeline(
                      ...
                      for (CacheData item : items) {
                        ...
                        if (!op.relNames.isEmpty()) {
                          p.sadd(
                              ...
                              op.relNames.toArray(new String[op.relNames.size()]));
  ...
  protected void evictItems(
    ...
    Failsafe.with(REDIS_RETRY_POLICY)
        .onRetriesExceeded(
            failure -> {
              throw new ExcessiveDynoFailureRetries(
                  format("Evicting items for %s:%s", prefix, type), failure);
            })
        .run(
            () ->
                redisClientDelegate.withPipeline(
                    pipeline -> {
                      ...
                      for (List<String> idPartition :
                          Lists.partition(identifiers, options.getMaxDelSize())) {
  ...
  protected Collection<CacheData> getItems(String type, List<String> ids, List<String> knownRels) {
    ...
    Map<String, Map<String, String>> rawItems =
        Failsafe.with(REDIS_RETRY_POLICY)
            .onRetriesExceeded(
                failure -> {
                  throw new ExcessiveDynoFailureRetries(
                      format("Getting items for %s:%s", prefix, type), failure);
                })
            .get(
                () ->
                    redisClientDelegate.withPipeline(
    ...
    cacheMetrics.get(
        prefix, type, results.size(), ids.size(), knownRels.size(), hmgetAllOperations.get());
    return results;
  }

  private CacheData extractHashedItem(
      String type, String id, Map<String, String> values, List<String> knownRels) {
    if (values == null) {
      return null;
    }
    ...
    try {
      ...
      if (values.get("attributes") != null) {
        attributes =
            objectMapper.readValue(
                compressionStrategy.decompress(values.get("attributes")), ATTRIBUTES);
      } else {
        attributes = null;
      }
      ...
      for (Map.Entry<String, String> value : values.entrySet()) {
        if (value.getKey().equals("attributes")
            || value.getKey().equals("id")
            || !knownRels.contains(value.getKey())) {
          continue;
        }
        ...
        try {
          ...
          log.warn(
              "Failed processing property '{}' on item '{}'", value.getKey(), itemId(type, id));
          continue;
        }
  ...
  protected Set<String> scanMembers(String setKey, Optional<String> glob) {
    return Failsafe.with(REDIS_RETRY_POLICY).get(() -> super.scanMembers(setKey, glob));
  }
  ...
  private MergeOp buildHashedMergeOp(String type, CacheData cacheData, Map<String, String> hashes) {
    ...
    return new MergeOp(
        cacheData.getRelationships().keySet(), valuesToSet, hashesToSet, skippedWrites);
  }

  private Map<CacheData, Map<String, String>> getAllHashes(
      String type, Collection<CacheData> items) {
    if (isHashingDisabled(type)) {
      return new HashMap<>();
    }

    return Failsafe.with(REDIS_RETRY_POLICY)
        .onRetriesExceeded(
            failure -> {
              throw new ExcessiveDynoFailureRetries(
                  format("Getting all requested hashes for %s:%s", prefix, type), failure);
            })
        .get(
            () ->
                redisClientDelegate.withPipeline(
  ...
  protected boolean isHashingDisabled(String type) {
    return Failsafe.with(REDIS_RETRY_POLICY)
        .onRetriesExceeded(
            failure -> {
              throw new ExcessiveDynoFailureRetries(
                  format("Getting hashing flag for %s:%s", prefix, type), failure);
            })
