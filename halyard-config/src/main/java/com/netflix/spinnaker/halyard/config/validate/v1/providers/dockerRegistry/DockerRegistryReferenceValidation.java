...
public class DockerRegistryReferenceValidation {
  public static void validateDockerRegistries(ConfigProblemSetBuilder psBuilder, DeploymentConfiguration deployment,
                                              List<String> dockerRegistryNames, final Provider.ProviderType providerType) {
    ...
    if (dockerRegistryProvider == null || dockerRegistryProvider.getAccounts() == null || dockerRegistryProvider
        .getAccounts().isEmpty()) {
      psBuilder.addProblem(ERROR, "The docker registry provider has not yet been configured for this deployment.",
          "dockerRegistries").setRemediation(providerType + " needs a Docker Registry as an image source to run.");
    } else if (!dockerRegistryProvider.isEnabled()) {
      psBuilder.addProblem(ERROR, "The docker registry provider needs to be enabled.");
    } else {
      List<String> availableRegistries = dockerRegistryProvider.getAccounts().stream().map(Account::getName)
      ...
      for (String registryName : dockerRegistryNames) {
        ...
        if (!availableRegistries.contains(registryName)) {
          psBuilder.addProblem(ERROR, "The chosen registry \"" + registryName
              + "\" has not been configured in your halconfig.", "dockerRegistries").setRemediation(
              "Either add \"" + registryName
