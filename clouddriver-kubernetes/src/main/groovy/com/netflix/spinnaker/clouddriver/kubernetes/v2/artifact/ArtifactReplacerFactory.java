...
public class ArtifactReplacerFactory {
  ...
  private static final String DOCKER_NAME_COMPONENT =
      "[a-z0-9]+(?:(?:(?:[._]|__|[-]*)[a-z0-9]+)+)?";
  private static final String DOCKER_OPTIONAL_TAG = "(?::[\\w][\\w.-]{0,127})?";
  private static final String DOCKER_OPTIONAL_DIGEST =
      "(?:@[A-Za-z][A-Za-z0-9]*(?:[-_+.][A-Za-z][A-Za-z0-9]*)*[:][0-9A-Fa-f]{32,})?";
  private static final String DOCKER_DOMAIN =
      "(?:[a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9-]*[a-zA-Z0-9])(?:(?:\\.(?:[a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9-]*[a-zA-Z0-9]))+)?";
  private static final String DOCKER_OPTIONAL_PORT = "(?::[0-9]+)?";
  private static final String DOCKER_OPTIONAL_DOMAIN_AND_PORT =
      "(?:" + DOCKER_DOMAIN + DOCKER_OPTIONAL_PORT + "/)?";
  private static final String DOCKER_IMAGE_NAME =
      "("
          ...
          + DOCKER_NAME_COMPONENT
          + ")*)";
  private static final String DOCKER_IMAGE_REFERENCE =
      DOCKER_IMAGE_NAME + "(" + DOCKER_OPTIONAL_TAG + "|" + DOCKER_OPTIONAL_DIGEST + ")";

  // the image reference pattern has two capture groups.
  // - the first captures the image name
  // - the second captures the image tag (including the leading ":") or digest (including the
  // leading "@").
  public static final Pattern DOCKER_IMAGE_REFERENCE_PATTERN =
      Pattern.compile("^" + DOCKER_IMAGE_REFERENCE + "$");
    ...
    return Replacer.builder()
        ...
        .nameFromReference(
            ref -> {
              ...
              // we don't need to check if this is a tag, or a port. ports will be matched lazily if
