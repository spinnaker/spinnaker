...
class CacheRepositoryTest {
  ...
  @BeforeEach
  void before() {
    CloudFoundryInstance instance = CloudFoundryInstance.builder()
      .appGuid("appguid")
      .key("abc123")
    ...
    CloudFoundryServerGroup serverGroup = CloudFoundryServerGroup.builder()
      .name("demo-dev-v001")
      .account("devaccount")
      .createdTime(1L)
      .space(CloudFoundrySpace.fromRegion("myorg > dev"))
      .instances(singleton(instance))
      .droplet(CloudFoundryDroplet.builder()
        .id("dropletid")
        .name("dropletname")
        .buildpacks(singletonList(CloudFoundryBuildpack.builder().buildpackName("java").build()))
        .sourcePackage(CloudFoundryPackage.builder()
          .checksum("check")
          .build())
        .build())
      .build();

    CloudFoundryCluster cluster = CloudFoundryCluster.builder().accountName("devaccount").name("demo-dev")
      .serverGroups(singleton(serverGroup)).build();
  ...
  void findApplication() {
    assertThat(repo.findApplicationByKey(Keys.getApplicationKey("demo"), FULL)).hasValueSatisfying(app -> {
      assertThat(app.getName()).isEqualTo("demo");
      assertThat(app.getClusters()).hasOnlyOneElementSatisfying(cluster -> {
        assertThat(cluster.getName()).isEqualTo("demo-dev");

        // rehydrated clusters are shallow, serve only the purpose of providing cluster names
  ...
  void findCluster() {
    ...
    assertThat(repo.findClusterByKey(clusterKey, FULL)).hasValueSatisfying(cluster -> {
      assertThat(cluster.getName()).isEqualTo("demo-dev");
      assertThat(cluster.getServerGroups()).hasOnlyOneElementSatisfying(serverGroup ->
        assertThat(serverGroup.getInstances()).hasSize(1)
      );
    });

    assertThat(repo.findClusterByKey(clusterKey, NAMES_ONLY)).hasValueSatisfying(cluster ->
      assertThat(cluster.getServerGroups()).hasOnlyOneElementSatisfying(serverGroup -> {
  ...
  void findServerGroup() {
    assertThat(repo.findServerGroupByKey(Keys.getServerGroupKey("devaccount", "demo-dev-v001", "myorg > dev"), FULL)).hasValueSatisfying(serverGroup -> {
      assertThat(serverGroup.getName()).isEqualTo("demo-dev-v001");
      assertThat(serverGroup.getInstances()).hasOnlyOneElementSatisfying(inst -> {
