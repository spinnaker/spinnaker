...
class GitEventHandlerSpec extends Specification implements RetrofitStubs {
  ...
  def pipelineTriggerConfiguration = Mock(PipelineTriggerConfiguration)

  @Subject
  def eventHandler = new GitEventHandler(registry, objectMapper, fiatPermissionEvaluator, pipelineTriggerConfiguration)
  ...
  @Unroll
  def "computes and compares GitHub signature, if available with a shared secret"() {
    def gitEvent = createGitEvent("github")
    gitEvent.rawContent = "toBeHashed"
    gitEvent.details.source = "github"
    if (signature) {
    ...
    def trigger = enabledGithubTrigger.atBranch("master")

    def pipeline = createPipelineWith(trigger)
    def pipelines = handlerSupport.pipelineCache(pipeline)
    pipelineTriggerConfiguration.getGitSharedSecret() >> secret
    ...
    null   | null                                       | 1
    "foo"  | null                                       | 0
    null   | "foo"                                      | 0 // No shared secret defined
    "foo"  | "foo"                                      | 0 // Signatures don't match
    "foo"  | "67af18bbedab68252b01902ac0a8d7095ca93692" | 1 // Signatures match! Generated by http://www.freeformatter.com/hmac-generator.html
  }
